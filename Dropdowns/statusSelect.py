...
class StatusSelect(discord.ui.Select):
    def __init__(self, user, owner):
        ...
        self.options = [
            discord.SelectOption(label="Online", description="Setzt den Status Online", emoji="🟢"),
            discord.SelectOption(label="DND", description="Setzt den Status Do not Disturb", emoji="⛔"),
            discord.SelectOption(label="Offline", description="Setzt den Status Offline"),
            discord.SelectOption(label="Abwesend", description="Setzt den Status Abwesend")
        ]
        super().__init__(placeholder="Wähle einen Status", max_values=1, min_values=1, options=self.options)
